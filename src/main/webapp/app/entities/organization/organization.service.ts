...
export class OrganizationService {
  ...
  protected convertDateFromClient(organization: IOrganization): IOrganization {
    const copy: IOrganization = Object.assign({}, organization, {
      createdDate: organization.createdDate && organization.createdDate.isValid() ? organization.createdDate.toJSON() : undefined,
      updatedDate: organization.updatedDate && organization.updatedDate.isValid() ? organization.updatedDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((organization: IOrganization) => {
        organization.createdDate = organization.createdDate ? moment(organization.createdDate) : undefined;
        organization.updatedDate = organization.updatedDate ? moment(organization.updatedDate) : undefined;
      });
    }
    return res;
  }
